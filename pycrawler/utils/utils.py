   ...
   def __treeSearch(output: Dict[str, Any], root: ElementTree.Element):
       for child in root:
           ...
           if len(child) > 0:
               ...
               output[child_name]["text"] = child.text.strip()
       return output
...
def treeDictToXml(tree_dict: Dict[str, Any]) -> ElementTree.Element:
    def __treeSearch(output: ElementTree.Element, root: Dict[str, Any]):
        ...
        return output
    xml_root = __treeSearch(ElementTree.Element(tree_dict.keys()[0]), tree_dict[tree_dict.keys()[0]])
    return xml_root
...
def formatDict(formatted: Dict[str, Any], original: Dict[str, Any], names: List[str] = []) -> Dict[str, Any]:
    for key, value in original.items():
        names.append(key)
        type_name = type(value).__name__
        if type_name == "dict":
            formatted = formatDict(formatted, value, names)
        else:
            name = ".".join(names)
            if name in formatted:
                if not formatted[name]["value"]: formatted[name] = { "type": type_name, "value": value }
            else:
                formatted[name] = { "type": type_name, "value": value }
        names.pop()
    return formatted